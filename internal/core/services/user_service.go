...
func NewUserService(repo ports.UserRepository, fileRepo ports.FileStorageRepository /*, emailSender ports.EmailSender */, roleRepo ports.RoleRepository, riverClient *river.Client[pgx.Tx]) *UserServiceImp {
	return &UserServiceImp{
		userRepo: repo,
		fileRepo: fileRepo,
		//emailSender: emailSender,
...
func (s *UserServiceImp) RegisterUser(user *domain.User) error {
	...
	emailJobArgs := jobs.SendEmailArgs{
		To:      newUser.Email,
		Subject: subject,
		Body:    body,
	}
	...
	// นำ Job เข้าไปในคิว
	_, err = s.riverClient.Insert(context.Background(), emailJobArgs, nil)
	if err != nil {
		// หากการนำเข้าคิวล้มเหลว ถือเป็นข้อผิดพลาดร้ายแรง
		// ควรบันทึก Log และพิจารณาว่าจะจัดการอย่างไรต่อไปตาม business logic ของคุณ
		log.Printf("CRITICAL: ไม่สามารถนำ Job ส่งอีเมลเข้าคิวได้: %v", err)
	}
